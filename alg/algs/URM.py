...
class URM(ERM):
    ...
    def _modify_generator_output(self):
        if self.args.urm_generator_output == "tanh":
            self.featurizer.activation = nn.Tanh()

        elif self.args.urm_generator_output == "sigmoid":
            self.featurizer.activation = nn.Sigmoid()

        elif self.args.urm_generator_output == "identity":
            self.featurizer.activation = nn.Identity()

        elif self.args.urm_generator_output == "relu":
            self.featurizer.activation = nn.ReLU()

        else:
            raise Exception(
                "unrecognized output activation: %s" % self.args.urm_generator_output
            )
    ...
    def _init_discriminator(self):
        ...
        """
        model = nn.Sequential()
        model.add_module("dense1", nn.Linear(self.featurizer.in_features, 100))
        model.add_module("act1", nn.LeakyReLU())

        for _ in range(self.args.urm_discriminator_hidden_layers):
    ...
    def _update_discriminator(self, x, y, feats):
        ...
        if self.args.urm_discriminator_label_smoothing:
            # label smoothing in discriminator
            soft_true_y = self._generate_soft_labels(
                noise.shape[0],
                noise.device,
                1 - self.args.urm_discriminator_label_smoothing,
                1.0,
            ...
            soft_fake_y = self._generate_soft_labels(
                ...
                feats.device,
                0,
                0 + self.args.urm_discriminator_label_smoothing,
            )  # random labels in range
            true_y = soft_true_y
            fake_y = soft_fake_y
        else:
        ...
        d_loss = 1 * noise_loss + self.args.urm_adv_lambda * feats_loss
    ...
    def _compute_loss(self, x, y):
        ...
        loss = ce_loss + self.args.urm_adv_lambda * g_loss
