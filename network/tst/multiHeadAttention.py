...
from network.tst.utils import generate_local_map_mask
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        self, d_model: int, q: int, v: int, h: int, attention_size: int = None
    ):
...
class MultiHeadAttentionChunk(MultiHeadAttention):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._future_mask = nn.Parameter(
            torch.triu(
                torch.ones((self._chunk_size, self._chunk_size)), diagonal=1
            ).bool(),
            requires_grad=False,
        )

        if self._attention_size is not None:
            self._attention_mask = nn.Parameter(
                generate_local_map_mask(self._chunk_size, self._attention_size),
                requires_grad=False,
            )
    ...
    def forward(
        ...
        # Compute Q, K and V, concatenate heads on batch dimension
        queries = torch.cat(
            torch.cat(self._W_q(query).chunk(self._h, dim=-1), dim=0).chunk(
                n_chunk, dim=1
            ),
            dim=0,
        )
        keys = torch.cat(
            torch.cat(self._W_k(key).chunk(self._h, dim=-1), dim=0).chunk(
                n_chunk, dim=1
            ),
            dim=0,
        )
        values = torch.cat(
            torch.cat(self._W_v(value).chunk(self._h, dim=-1), dim=0).chunk(
                n_chunk, dim=1
            ),
            dim=0,
        )
        ...
        # Scaled Dot Product
        self._scores = torch.bmm(queries, keys.transpose(1, 2)) / np.sqrt(
            self._chunk_size
        )
        ...
        attention_heads = torch.cat(
            torch.cat(attention.chunk(n_chunk, dim=0), dim=1).chunk(self._h, dim=0),
            dim=-1,
        )
...
class MultiHeadAttentionWindow(MultiHeadAttention):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._future_mask = nn.Parameter(
            torch.triu(
                torch.ones((self._window_size, self._window_size)), diagonal=1
            ).bool(),
            requires_grad=False,
        )
        ...
        if self._attention_size is not None:
            self._attention_mask = nn.Parameter(
                generate_local_map_mask(self._window_size, self._attention_size),
                requires_grad=False,
            )
    ...
    def forward(
        ...
        query = F.pad(
            query.transpose(1, 2), (self._padding, self._padding), "replicate"
        ).transpose(1, 2)
        key = F.pad(
            key.transpose(1, 2), (self._padding, self._padding), "replicate"
        ).transpose(1, 2)
        value = F.pad(
            value.transpose(1, 2), (self._padding, self._padding), "replicate"
        ).transpose(1, 2)
        ...
        queries = (
            queries.unfold(dimension=1, size=self._window_size, step=self._step)
            .reshape((-1, self._q, self._window_size))
            .transpose(1, 2)
        )
        keys = (
            keys.unfold(dimension=1, size=self._window_size, step=self._step)
            .reshape((-1, self._q, self._window_size))
            .transpose(1, 2)
        )
        values = (
            values.unfold(dimension=1, size=self._window_size, step=self._step)
            .reshape((-1, self._v, self._window_size))
            .transpose(1, 2)
        )
